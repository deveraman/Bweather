...
import 'package:weather_repository/src/models/alert/alert.dart';
...
void main() {
  ...
  group('Alert model', () {
    final Alert alert = Alert.fromJson(response);
    ...
    test('supports value equality in serialized json', () {
      expect(Alert.fromJson(response), equals(alert));
    });

    test('copyWith updates fields', () {
      final newAlert = alert.copyWith(severity: "High");

      expect(newAlert.severity, equals("High"));
      expect(newAlert, isNot(equals(alert)));
    });

    test('deserializes json', () {
      final des = alert.toJson();
